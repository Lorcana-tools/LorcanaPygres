...
def lookup_card_details_by_row(card_row):
    ...
    card_name, card_subtitle, ink_color, card_identifier, ink_cost, inkable = query['name'][0].strip(), query['subtitle'][0], query['ink_color'][0], query['card_identifier'][0], int(query['ink_cost'][0]), bool(query['card_identifier'][0])
    if card_subtitle is not None:
        card_name = card_name + ' - ' + card_subtitle.strip()
